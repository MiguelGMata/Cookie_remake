...
(function () {
  ...
  var WATER_BOTTOM_COLOR = "white"; //color ondas
  ...
  function value(x, width, numberOfWaves) {
    x = x * numberOfWaves / width * 0.5 * Math.PI; //multiplica  ondas
    return Math.sin(x);
  }

  function multiplier(x, width) {
    var multiplierVar = 80; //profundida ondas
  ...
  function draw() {
    ...
    gradient.addColorStop(0.7, WATER_TOP_COLOR); //degrade
    gradient.addColorStop(0.9, WATER_BOTTOM_COLOR); //degrade
    ctx.fillStyle = gradient;
    ctx.lineTo(0, screenHeight); //alineacion borde izquierdo
    ctx.fill();
  ...
  function moveShip() {
    ...
    var y2 = waves[shipComputed.left + 2]; //grados gira la imagen

    shipComputed.angle = -Math.atan(y1 - y2) + 'rad';
    shipComputed.top = y1;
    shipComputed.left += 3; //rapidez cookie
  ...
  function initializeWaves() {
    var randomWaves1 = randomIntFromInterval(5, 5); //intervalos
    var randomWaves2 = randomIntFromInterval(2, 3);
    var randomWaves3 = randomIntFromInterval(6, 7);//intervalos ondas pequenas
